...
use BlackCat\Database\Packages\Permissions\Dto\PermissionDto as Dto;
use BlackCat\Database\Packages\Permissions\Mapper\PermissionDtoMapper as RowMapper;
...
final class PermissionRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Permissions\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Permissions\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'description' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Permissions\Dto\PermissionDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\Permissions\Dto\PermissionDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Permissions\Mapper\PermissionDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Permissions\Dto\PermissionDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Permissions\Dto\PermissionDto|null {
