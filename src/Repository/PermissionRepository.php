...
use BlackCat\Database\Packages\Permissions\Repository\PermissionRepositoryInterface;
...
class PermissionRepository implements PermissionRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Permissions\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'description' ]);
          return count($rows);
        }
