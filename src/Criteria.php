 ...
 * - whitelist filtrů: [ 'id', 'name', 'description', 'created_at', 'updated_at' ]
 * - whitelist pro LIKE hledání: [ 'name', 'description' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'name', 'description', 'created_at', 'updated_at' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'name', 'description', 'created_at', 'updated_at' ], true);
            $parts = [];
